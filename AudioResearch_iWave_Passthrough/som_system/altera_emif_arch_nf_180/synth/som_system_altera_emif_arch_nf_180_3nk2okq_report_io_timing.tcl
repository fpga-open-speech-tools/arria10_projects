...
source "$script_dir/som_system_altera_emif_arch_nf_180_3nk2okq_report_timing_core.tcl"


if [ info exists ddr_db ] {
   unset ddr_db
}
som_system_altera_emif_arch_nf_180_3nk2okq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_som_system_altera_emif_arch_nf_180_3nk2okq_corename}"
   } else {
      set fbasename "${::GLOBAL_som_system_altera_emif_arch_nf_180_3nk2okq_corename}_${inst_id}"
   }
   ...
   som_system_altera_emif_arch_nf_180_3nk2okq_perform_read_capture_analysis $opcname $inst pins var summary
   
   #######################################
   # Write Analysis

   som_system_altera_emif_arch_nf_180_3nk2okq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   som_system_altera_emif_arch_nf_180_3nk2okq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   som_system_altera_emif_arch_nf_180_3nk2okq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis 
   
   som_system_altera_emif_arch_nf_180_3nk2okq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # PHY Analyses
   
   som_system_altera_emif_arch_nf_180_3nk2okq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command som_system_altera_emif_arch_nf_180_3nk2okq_sort_proc $summary]
   ...
   puts $f "Core: ${::GLOBAL_som_system_altera_emif_arch_nf_180_3nk2okq_corename} - Instance: $inst"
   puts $f "Path, Setup Margin, Hold Margin"

   
   post_message -type info "Core: ${::GLOBAL_som_system_altera_emif_arch_nf_180_3nk2okq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ som_system_altera_emif_arch_nf_180_3nk2okq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ som_system_altera_emif_arch_nf_180_3nk2okq_round_3dp $hold]
      }
