...
source "$script_dir/soc_system_altera_emif_arch_nf_180_hlqzivq_report_timing_core.tcl"


if [ info exists ddr_db ] {
   unset ddr_db
}
soc_system_altera_emif_arch_nf_180_hlqzivq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_soc_system_altera_emif_arch_nf_180_hlqzivq_corename}"
   } else {
      set fbasename "${::GLOBAL_soc_system_altera_emif_arch_nf_180_hlqzivq_corename}_${inst_id}"
   }
   ...
   soc_system_altera_emif_arch_nf_180_hlqzivq_perform_read_capture_analysis $opcname $inst pins var summary
   
   #######################################
   # Write Analysis

   soc_system_altera_emif_arch_nf_180_hlqzivq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   soc_system_altera_emif_arch_nf_180_hlqzivq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   soc_system_altera_emif_arch_nf_180_hlqzivq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis 
   
   soc_system_altera_emif_arch_nf_180_hlqzivq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # PHY Analyses
   
   soc_system_altera_emif_arch_nf_180_hlqzivq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command soc_system_altera_emif_arch_nf_180_hlqzivq_sort_proc $summary]
   ...
   puts $f "Core: ${::GLOBAL_soc_system_altera_emif_arch_nf_180_hlqzivq_corename} - Instance: $inst"
   puts $f "Path, Setup Margin, Hold Margin"

   
   post_message -type info "Core: ${::GLOBAL_soc_system_altera_emif_arch_nf_180_hlqzivq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ soc_system_altera_emif_arch_nf_180_hlqzivq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ soc_system_altera_emif_arch_nf_180_hlqzivq_round_3dp $hold]
      }
